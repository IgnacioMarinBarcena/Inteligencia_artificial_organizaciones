...
class JugadorAgent_Jaime(Agent):
    ...
    class Unir_jugador_partida(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            ...
            self.tipo = JugadorAgent_Jaime.TipoIntencion.PEDIR_UNIRSE
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.UNIDO_A_PARTIDA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.RECHAZADO_A_PARTIDA:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    tid = c.valores[0]
                    ...
                    miCreencia = JugadorAgent_Jaime.Creencia(
                        JugadorAgent_Jaime.TipoCreencia.PEDIDO_UNIRSE, tid)
    ...
    class Solicitar_rol(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Jaime.TipoIntencion.PEDIR_ROL
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ROL_ASIGNADO:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ROL_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Jaime.Creencia(
                        JugadorAgent_Jaime.TipoCreencia.ROL_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Mover_ficha(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Jaime.TipoIntencion.PEDIR_MOVER_FICHA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.CIUDADES_DEVUELTAS:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.CIUDADES_RECHAZADAS:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Jaime.Creencia(
                        JugadorAgent_Jaime.TipoCreencia.CIUDADES_PEDIDAS, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Mover_ficha_a(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Jaime.TipoIntencion.PEDIR_MOVER_FICHA_A
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.FICHA_MOVIDA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.MOVIMIENTO_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    ...
                    contador_rojo = 0
                    contador_azul = 0
                    for ciudad in global_.ciudades:
                        if ciudad.color == "Rojo":
                            contador_rojo += ciudad.cubos
                        if ciudad.color == "Azul":
                            contador_azul += ciudad.cubos
                    if contador_azul < contador_rojo:
                        enfermedad_eliminar = "Azul"
                    elif contador_azul > contador_rojo:
                        enfermedad_eliminar = "Rojo"
                    else:
                    ...
                    for c in global_.adyacentes:
                        if c.color == enfermedad_eliminar:
                            cubos.append(c.cubos)
                    numM = max(cubos)
                    ...
                    miCreencia = JugadorAgent_Jaime.Creencia(
                        JugadorAgent_Jaime.TipoCreencia.MOVIMIENTO_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Robar_carta(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Jaime.TipoIntencion.PEDIR_ROBAR_CARTA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.CARTA_ROBADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ROBOCARTA_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Jaime.Creencia(
                        JugadorAgent_Jaime.TipoCreencia.ROBOCARTA_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Descartar_carta(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Jaime.TipoIntencion.PEDIR_DESCARTAR_CARTA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.CARTA_DESCARTADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.DESCARTECARTA_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Jaime.Creencia(
                        JugadorAgent_Jaime.TipoCreencia.DESCARTECARTA_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Tratar_enfermedad(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Jaime.TipoIntencion.PEDIR_TRATAR_ENFERMEDAD
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ENFERMEDAD_TRATADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.TRATOENF_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Jaime.Creencia(
                        JugadorAgent_Jaime.TipoCreencia.TRATOENF_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Jugar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.tipo = JugadorAgent_Jaime.TipoDeseo.JUGAR
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.PEDIDO_UNIRSE:
                    return True
            return False
    ...
    class Rol(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Jaime.TipoDeseo.ROL
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ROL_PEDIDO:
                    return True
            return False
    ...
    class Robar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Jaime.TipoDeseo.ROBAR_CARTA
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ROBOCARTA_PEDIDO:
                    return True
            return False
    ...
    class Descartar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Jaime.TipoDeseo.DESCARTAR_CARTA
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.DESCARTECARTA_PEDIDO:
                    return True
            return False
    ...
    class Moverficha(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Jaime.TipoDeseo.MOVER_FICHA

        # Comprobar que el deseo no es imposible realizarlo
        def comprobarimposible(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.CIUDADES_RECHAZADAS or c.tipo == JugadorAgent_Jaime.TipoCreencia.MOVIMIENTO_RECHAZADO:
                    return True
            return False
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.CIUDADES_PEDIDAS or c.tipo == JugadorAgent_Jaime.TipoCreencia.MOVIMIENTO_PEDIDO:
                    return True
            return False
        ...
        def comprobaractivar(self, creencias):
            ...
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.CARTA_DESCARTADA:
                    pedido = True
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.MOVIMIENTO_PEDIDO:
                    pedido = False
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.ES_TABLERO:
                    tablero_conocido = True
            return pedido and tablero_conocido
    ...
    class Tratarenfermedad(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Jaime.TipoDeseo.TRATAR_ENFERMEDAD
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Jaime.TipoCreencia.TRATOENF_PEDIDO:
                    return True
            return False
    ...
    async def setup(self):
        print("JugadorAgent_Jaime iniciado")
        self.tablero_Id = "tablerog5@jabbers.one"
        self.misCreencias = []
        self.misIntenciones = []
        self.misDeseos = []
        
        miCreencia = self.Creencia(JugadorAgent_Jaime.TipoCreencia.ES_TABLERO, self.tablero_Id)
        self.misCreencias.append(miCreencia)
