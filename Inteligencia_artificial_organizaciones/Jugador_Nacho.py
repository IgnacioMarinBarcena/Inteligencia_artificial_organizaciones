...
class JugadorAgent_Nacho(Agent):
    ...
    class Unir_jugador_partida(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            ...
            self.tipo = JugadorAgent_Nacho.TipoIntencion.PEDIR_UNIRSE
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.UNIDO_A_PARTIDA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.RECHAZADO_A_PARTIDA:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    tid = c.valores[0]
                    ...
                    miCreencia = JugadorAgent_Nacho.Creencia(
                        JugadorAgent_Nacho.TipoCreencia.PEDIDO_UNIRSE, tid)
    ...
    class Solicitar_rol(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Nacho.TipoIntencion.PEDIR_ROL
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ROL_ASIGNADO:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ROL_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Nacho.Creencia(
                        JugadorAgent_Nacho.TipoCreencia.ROL_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Mover_ficha(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Nacho.TipoIntencion.PEDIR_MOVER_FICHA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.CIUDADES_DEVUELTAS:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.CIUDADES_RECHAZADAS:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Nacho.Creencia(
                        JugadorAgent_Nacho.TipoCreencia.CIUDADES_PEDIDAS, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Mover_ficha_a(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Nacho.TipoIntencion.PEDIR_MOVER_FICHA_A
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.FICHA_MOVIDA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.MOVIMIENTO_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Nacho.Creencia(
                        JugadorAgent_Nacho.TipoCreencia.MOVIMIENTO_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Robar_carta(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Nacho.TipoIntencion.PEDIR_ROBAR_CARTA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.CARTA_ROBADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ROBOCARTA_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Nacho.Creencia(
                        JugadorAgent_Nacho.TipoCreencia.ROBOCARTA_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Descartar_carta(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Nacho.TipoIntencion.PEDIR_DESCARTAR_CARTA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.CARTA_DESCARTADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.DESCARTECARTA_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Nacho.Creencia(
                        JugadorAgent_Nacho.TipoCreencia.DESCARTECARTA_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Tratar_enfermedad(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Nacho.TipoIntencion.PEDIR_TRATAR_ENFERMEDAD
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ENFERMEDAD_TRATADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.TRATOENF_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Nacho.Creencia(
                        JugadorAgent_Nacho.TipoCreencia.TRATOENF_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Jugar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.tipo = JugadorAgent_Nacho.TipoDeseo.JUGAR
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.PEDIDO_UNIRSE:
                    return True
            return False
    ...
    class Rol(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Nacho.TipoDeseo.ROL
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ROL_PEDIDO:
                    return True
            return False
    ...
    class Robar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Nacho.TipoDeseo.ROBAR_CARTA
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ROBOCARTA_PEDIDO:
                    return True
            return False
    ...
    class Descartar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Nacho.TipoDeseo.DESCARTAR_CARTA
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.DESCARTECARTA_PEDIDO:
                    return True
            return False
    ...
    class Moverficha(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Nacho.TipoDeseo.MOVER_FICHA

        # Comprobar que el deseo no es imposible realizarlo
        def comprobarimposible(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.CIUDADES_RECHAZADAS or c.tipo == JugadorAgent_Nacho.TipoCreencia.MOVIMIENTO_RECHAZADO:
                    return True
            return False
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.CIUDADES_PEDIDAS or c.tipo == JugadorAgent_Nacho.TipoCreencia.MOVIMIENTO_PEDIDO:
                    return True
            return False
        ...
        def comprobaractivar(self, creencias):
            ...
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.CARTA_DESCARTADA:
                    pedido = True
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.MOVIMIENTO_PEDIDO:
                    pedido = False
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.ES_TABLERO:
                    tablero_conocido = True
            return pedido and tablero_conocido
    ...
    class Tratarenfermedad(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Nacho.TipoDeseo.TRATAR_ENFERMEDAD
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Nacho.TipoCreencia.TRATOENF_PEDIDO:
                    return True
            return False
    ...
    async def setup(self):
        print("JugadorAgent_Nacho iniciado")
        self.tablero_Id = "tablerog5@jabbers.one"
        self.misCreencias = []
        self.misIntenciones = []
        self.misDeseos = []
        
        miCreencia = self.Creencia(JugadorAgent_Nacho.TipoCreencia.ES_TABLERO, self.tablero_Id)
        self.misCreencias.append(miCreencia)
