...
class JugadorAgent_Martin(Agent):
    ...
    class Unir_jugador_partida(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            ...
            self.tipo = JugadorAgent_Martin.TipoIntencion.PEDIR_UNIRSE
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.UNIDO_A_PARTIDA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.RECHAZADO_A_PARTIDA:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    tid = c.valores[0]
                    ...
                    miCreencia = JugadorAgent_Martin.Creencia(
                        JugadorAgent_Martin.TipoCreencia.PEDIDO_UNIRSE, tid)
    ...
    class Solicitar_rol(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Martin.TipoIntencion.PEDIR_ROL
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ROL_ASIGNADO:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ROL_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Martin.Creencia(
                        JugadorAgent_Martin.TipoCreencia.ROL_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Mover_ficha(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Martin.TipoIntencion.PEDIR_MOVER_FICHA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.CIUDADES_DEVUELTAS:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.CIUDADES_RECHAZADAS:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Martin.Creencia(
                        JugadorAgent_Martin.TipoCreencia.CIUDADES_PEDIDAS, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Mover_ficha_a(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Martin.TipoIntencion.PEDIR_MOVER_FICHA_A
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.FICHA_MOVIDA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.MOVIMIENTO_RECHAZADO:
                    return True
            return False
            ...
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    ...
                    for c in global_.adyacentes:
                        for carta in global_.mazo_cartas:
                            if c.nombre == carta.nombre:
                                ciudadesC.append(c)
                    if len(ciudadesC) > 0:
                        index = random.randint(0, len(ciudadesC)-1)
                        global_.destino = ciudadesC[index].nombre
                    else:              
                        index = random.randint(0, len(global_.adyacentes)-1)
                        global_.destino = global_.adyacentes[index].nombre

                    miCreencia = JugadorAgent_Martin.Creencia(
                        JugadorAgent_Martin.TipoCreencia.MOVIMIENTO_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Robar_carta(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Martin.TipoIntencion.PEDIR_ROBAR_CARTA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.CARTA_ROBADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ROBOCARTA_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Martin.Creencia(
                        JugadorAgent_Martin.TipoCreencia.ROBOCARTA_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Descartar_carta(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Martin.TipoIntencion.PEDIR_DESCARTAR_CARTA
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.CARTA_DESCARTADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.DESCARTECARTA_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Martin.Creencia(
                        JugadorAgent_Martin.TipoCreencia.DESCARTECARTA_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Tratar_enfermedad(Intencion):
        def __init__(self, conceptos_intencion, deseo_intencion, posteriores):
            # Conceptos necesarios para ejecutar la intencion
            self.conceptos = conceptos_intencion
            self.tipo = JugadorAgent_Martin.TipoIntencion.PEDIR_TRATAR_ENFERMEDAD
            self.deseo = deseo_intencion
            self.coste = 0
            self.urgencia = 0
            self.posteriores = []  # Lista con intenciones que puede que necesiten ejecutarse despues
        ...
        def comprobaralcanzada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ENFERMEDAD_TRATADA:
                    return True
            return False
        ...
        def comprobaranulada(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.TRATOENF_RECHAZADO:
                    return True
            return False
        ...
        async def ejecuta(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    ...
                    miCreencia = JugadorAgent_Martin.Creencia(
                        JugadorAgent_Martin.TipoCreencia.TRATOENF_PEDIDO, tid)
                    return msg, miCreencia  # Devuelve el mensaje y la creencia para que puedan ser tratados
    ...
    class Jugar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.tipo = JugadorAgent_Martin.TipoDeseo.JUGAR
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.PEDIDO_UNIRSE:
                    return True
            return False
    ...
    class Rol(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Martin.TipoDeseo.ROL
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ROL_PEDIDO:
                    return True
            return False
    ...
    class Robar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Martin.TipoDeseo.ROBAR_CARTA
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ROBOCARTA_PEDIDO:
                    return True
            return False
    ...
    class Descartar(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Martin.TipoDeseo.DESCARTAR_CARTA
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.DESCARTECARTA_PEDIDO:
                    return True
            return False
    ...
    class Moverficha(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Martin.TipoDeseo.MOVER_FICHA

        # Comprobar que el deseo no es imposible realizarlo
        def comprobarimposible(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.CIUDADES_RECHAZADAS or c.tipo == JugadorAgent_Martin.TipoCreencia.MOVIMIENTO_RECHAZADO:
                    return True
            return False
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.CIUDADES_PEDIDAS or c.tipo == JugadorAgent_Martin.TipoCreencia.MOVIMIENTO_PEDIDO:
                    return True
            return False
        ...
        def comprobaractivar(self, creencias):
            ...
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.CARTA_DESCARTADA:
                    pedido = True
                if c.tipo == JugadorAgent_Martin.TipoCreencia.MOVIMIENTO_PEDIDO:
                    pedido = False
                if c.tipo == JugadorAgent_Martin.TipoCreencia.ES_TABLERO:
                    tablero_conocido = True
            return pedido and tablero_conocido
    ...
    class Tratarenfermedad(Deseo):
        def __init__(self, intenciones_deseo, conceptos_deseo):
            ...
            self.conceptos = conceptos_deseo
            self.activo = False  # Booleano que representa si el deseo esta activo o no
            self.tipo = JugadorAgent_Martin.TipoDeseo.TRATAR_ENFERMEDAD
        ...
        def comprobarinteres(self, creencias):
            for c in creencias:
                if c.tipo == JugadorAgent_Martin.TipoCreencia.TRATOENF_PEDIDO:
                    return True
            return False
    ...
    def actualizaCreencias(self, msg):
        if msg.body == "Unirme" and msg.get_metadata("performative") == "agree":
            miCreencia = JugadorAgent_Martin.Creencia(
                JugadorAgent_Martin.TipoCreencia.UNIDO_A_PARTIDA, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Unirme" and msg.get_metadata("performative") == "refuse":
            miCreencia = self.Creencia(
                JugadorAgent_Martin.TipoCreencia.RECHAZADO_A_PARTIDA, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Necesito rol" and msg.get_metadata("performative") == "agree":
            miCreencia = JugadorAgent_Martin.Creencia(
                JugadorAgent_Martin.TipoCreencia.ROL_ASIGNADO, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Necesito rol" and msg.get_metadata("performative") == "refuse":
            miCreencia = self.Creencia(
                JugadorAgent_Martin.TipoCreencia.ROL_RECHAZADO, str(msg.sender))
            self.misCreencias.append(miCreencia)
        
        if msg.body == "Mueve ficha" and msg.get_metadata("performative") == "agree":
            miCreencia = JugadorAgent_Martin.Creencia(
                JugadorAgent_Martin.TipoCreencia.CIUDADES_DEVUELTAS, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Mueve ficha" and msg.get_metadata("performative") == "refuse":
            miCreencia = self.Creencia(
                JugadorAgent_Martin.TipoCreencia.CIUDADES_RECHAZADAS, str(msg.sender))
            self.misCreencias.append(miCreencia)
        
        if msg.body == "Roba carta" and msg.get_metadata("performative") == "agree":
            miCreencia = JugadorAgent_Martin.Creencia(
                JugadorAgent_Martin.TipoCreencia.CARTA_ROBADA, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Roba carta" and msg.get_metadata("performative") == "refuse":
            miCreencia = self.Creencia(
                JugadorAgent_Martin.TipoCreencia.ROBOCARTA_RECHAZADO, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Descarta carta" and msg.get_metadata("performative") == "agree":
            miCreencia = JugadorAgent_Martin.Creencia(
                JugadorAgent_Martin.TipoCreencia.CARTA_DESCARTADA, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Descarta carta" and msg.get_metadata("performative") == "refuse":
            miCreencia = self.Creencia(
                JugadorAgent_Martin.TipoCreencia.DESCARTECARTA_RECHAZADO, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Mueve ficha a" and msg.get_metadata("performative") == "agree":
            miCreencia = JugadorAgent_Martin.Creencia(
                JugadorAgent_Martin.TipoCreencia.FICHA_MOVIDA, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Mueve ficha a" and msg.get_metadata("performative") == "refuse":
            miCreencia = self.Creencia(
                JugadorAgent_Martin.TipoCreencia.MOVIMIENTO_RECHAZADO, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Trata enfermedad" and msg.get_metadata("performative") == "agree":
            miCreencia = JugadorAgent_Martin.Creencia(
                JugadorAgent_Martin.TipoCreencia.ENFERMEDAD_TRATADA, str(msg.sender))
            self.misCreencias.append(miCreencia)

        if msg.body == "Trata enfermedad" and msg.get_metadata("performative") == "refuse":
            miCreencia = self.Creencia(
                JugadorAgent_Martin.TipoCreencia.TRATOENF_RECHAZADO, str(msg.sender))
            self.misCreencias.append(miCreencia)
    ...
    async def setup(self):
        print("JugadorAgent_Martin iniciado")
        self.tablero_Id = "tablerog5@jabbers.one"
        self.misCreencias = []
        self.misIntenciones = []
        self.misDeseos = []
        
        miCreencia = self.Creencia(JugadorAgent_Martin.TipoCreencia.ES_TABLERO, self.tablero_Id)
        self.misCreencias.append(miCreencia)
